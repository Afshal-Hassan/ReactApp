...
export default function Login() {
            ...
            axios.post(url, bodyform, {
                ...
                .then((res) => {
                    ...
                    if (res.status == 200) {
                        ...
                        setTimeout(function () {
                            document.getElementById("loader").style.cssText = "display:initial";
                        }, 10)
                        setTimeout(function () {
    ...
    return (
        <div id="Login-Module">
            ...
            <h1>!!تسجيل الدخول</h1>
            <form>
                <input type="email" placeholder="البريد الإلكتروني" name="email" id="name-login" value={emailLogin} onChange={onchangeLoginName} />
                <input type="password" placeholder="شعار" name="password" id="password-login" value={passwordLogin} onChange={onchangeLoginPassword} />
                <Link to="/signup-arabic" id="href-signup">الاشتراك؟</Link>
                <Button value="Submit" variant="outlined" type="submit" color="primary" id="Login-Submit" disabled={disabledValue} onClick={LoginHandler}>تسجيل الدخول</Button>
            </form>
            <ToastContainer />
        </div>);
